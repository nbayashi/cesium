...
function FullscreenButtonViewModel(fullscreenElement, container) {
  ...
  this._fullscreenElement = getElement3(fullscreenElement) ?? ownerDocument.body;
  ...
  fullscreenElement: {
    ...
    set: function(value) {
      if (!(value instanceof Element)) {
        throw new DeveloperError9("value must be a valid Element.");
      }
...
function GeocoderViewModel(options) {
  if (!defined12(options) || !defined12(options.scene)) {
    throw new DeveloperError11("options.scene is required.");
  }
  if (defined12(options.geocoderServices)) {
  ...
  this._searchCommand = createCommand_default(function(geocodeType) {
    ...
    if (defined12(that._selectedSuggestion)) {
  ...
  knockout_default.defineProperty(this, "searchText", {
    ...
    set: function(value) {
      if (typeof value !== "string") {
        throw new DeveloperError11("value must be a valid string.");
      }
...
function computeFlyToLocationForCartographic(cartographic, terrainProvider) {
  const availability = defined12(terrainProvider) ? terrainProvider.availability : void 0;
  if (!defined12(availability)) {
...
async function geocode(viewModel, geocoderServices, geocodeType) {
  ...
  for (i = 0; i < geocoderServices.length; i++) {
    ...
    if (defined12(result) && result.state === "fulfilled" && result.value.length > 0) {
      break;
    }
  }
  if (viewModel._wasGeocodeCancelled) {
  ...
  if (result.state === "fulfilled" && defined12(geocoderResults) && geocoderResults.length > 0) {
...
function updateCredit(viewModel, credit) {
  if (defined12(credit) && !viewModel._scene.isDestroyed() && !viewModel._scene.frameState.creditDisplay.isDestroyed()) {
...
function adjustSuggestionsScroll(viewModel, focusedItemIndex) {
  const container = getElement5(viewModel._viewContainer);
  ...
  for (const service of viewModel._geocoderServices) {
    ...
    if (newResults.length > 0) {
      ...
      newResults.forEach((result) => {
        ...
        useDefaultCredit = useDefaultCredit && !defined12(credits);
...
function Geocoder(options) {
  if (!defined13(options) || !defined13(options.container)) {
    throw new DeveloperError12("options.container is required.");
  }
  if (!defined13(options.scene)) {
    throw new DeveloperError12("options.scene is required.");
  }
  const container = getElement6(options.container);
...
function CesiumInspectorViewModel(scene, performanceContainer) {
  if (!defined14(scene)) {
    throw new DeveloperError13("scene is required");
  }
  if (!defined14(performanceContainer)) {
    throw new DeveloperError13("performanceContainer is required");
  }
  const that = this;
  ...
  this._showPrimitiveReferenceFrame = createCommand_default(function() {
    ...
    } else if (defined14(that._modelMatrixPrimitive)) {
  ...
  this._doFilterPrimitive = createCommand_default(function() {
    if (that.filterPrimitive) {
      that._scene.debugCommandFilter = function(command) {
        if (defined14(that._modelMatrixPrimitive) && command.owner === that._modelMatrixPrimitive._primitive) {
          return true;
        } else if (defined14(that._primitive)) {
  ...
  this._showTileCoordinates = createCommand_default(function() {
    if (that.tileCoordinates && !defined14(tileBoundariesLayer)) {
    ...
    } else if (!that.tileCoordinates && defined14(tileBoundariesLayer)) {
  ...
  this._doFilterTile = createCommand_default(function() {
    if (!that.filterTile) {
      ...
      if (defined14(that._tile) && that._tile.renderable) {
  ...
  function pickPrimitive(e) {
    ...
    if (defined14(newPick)) {
      that.primitive = defined14(newPick.collection) ? newPick.collection : newPick.primitive;
    }
    that._scene.requestRender();
  ...
  function selectTile(e) {
    ...
    if (defined14(cartesian)) {
      ...
      for (let textureCount = 0; !selectedTile && textureCount < tilesRendered.length; ++textureCount) {
        ...
        if (!defined14(tilesRenderedByTextureCount)) {
          continue;
        }
        for (let tileIndex = 0; !selectedTile && tileIndex < tilesRenderedByTextureCount.length; ++tileIndex) {
          ...
          if (Rectangle2.contains(tile.rectangle, cartographic)) {
            selectedTile = tile;
          }
        }
      }
    ...
    set: function(newTile) {
      if (defined14(newTile)) {
        ...
        if (newTile !== oldTile) {
          ...
          if (defined14(data) && defined14(data.tileBoundingRegion)) {
...
function CesiumInspector(container, scene) {
  if (!defined15(container)) {
    throw new DeveloperError14("container is required.");
  }
  if (!defined15(scene)) {
    throw new DeveloperError14("scene is required.");
  }
  container = getElement7(container);
...
import { defined as defined16, DeveloperError as DeveloperError15 } from "@cesium/engine";
...
import { Check as Check3, destroyObject as destroyObject12, getElement as getElement9 } from "@cesium/engine";
...
function getPickTileset(viewModel) {
  return function(e) {
    ...
    if (defined18(pick) && pick.primitive instanceof Cesium3DTileset) {
...
function selectTilesetOnHover(viewModel, value) {
  if (value) {
    ...
    }, ScreenSpaceEventType2.MOUSE_MOVE);
  } else {
    viewModel._eventHandler.removeInputAction(ScreenSpaceEventType2.MOUSE_MOVE);
...
function Cesium3DTilesInspectorViewModel(scene, performanceContainer) {
  ...
  const that = this;
  const canvas = scene.canvas;
  this._eventHandler = new ScreenSpaceEventHandler2(canvas);
  this._scene = scene;
  this._performanceContainer = performanceContainer;
  this._canvas = canvas;
  this._performanceDisplay = new PerformanceDisplay2({
    ...
    set: function(value) {
      ...
      if (value) {
        that._eventHandler.setInputAction(function(e) {
          ...
          } else if (defined18(picked) && defined18(picked.content)) {
            that.feature = void 0;
            that.tile = picked.content.tile;
          } else {
            that.feature = void 0;
          ...
          if (showOnlyPickedTileDebugLabel && defined18(picked) && defined18(picked.content)) {
            let position;
            if (scene.pickPositionSupported) {
              position = scene.pickPosition(e.endPosition);
              if (defined18(position)) {
        ...
        that.feature = void 0;
        that.tile = void 0;
        that._eventHandler.removeInputAction(ScreenSpaceEventType2.MOUSE_MOVE);
      }
    }
  });
  this.picking = true;
  ...
  knockout_default.defineProperty(this, "pickActive", {
    ...
    set: function(value) {
      ...
      if (value) {
        ...
        that._eventHandler.removeInputAction(ScreenSpaceEventType2.LEFT_CLICK);
      }
    }
  });
...
Object.defineProperties(Cesium3DTilesInspectorViewModel.prototype, {
  ...
  feature: {
    ...
    set: function(feature) {
      ...
      if (defined18(currentFeature) && !currentFeature.content.isDestroyed()) {
        if (!this.colorize && defined18(this._style)) {
          currentFeature.color = defined18(this._style.color) ? this._style.color.evaluateColor(currentFeature, scratchColor) : Color2.WHITE;
        } else {
          currentFeature.color = oldColor;
        }
        this._scene.requestRender();
      }
      if (defined18(feature)) {
  ...
  tile: {
    ...
    set: function(tile) {
      ...
      if (defined18(currentTile) && !currentTile.isDestroyed() && !hasFeatures(currentTile.content)) {
        currentTile.color = oldColor;
        this._scene.requestRender();
      }
      if (defined18(tile) && !hasFeatures(tile.content)) {
...
Cesium3DTilesInspectorViewModel.prototype.compileStyle = function() {
  ...
  if (!defined18(tileset) || this.styleString === JSON.stringify(tileset.style)) {
    return;
  }
  this._editorError = "";
  try {
...
function ProjectionPickerViewModel(scene) {
  ...
  this._eventHelper.add(scene.preRender, function() {
    that._flightInProgress = defined28(scene.camera._currentFlight);
  });
...
function lockScreen(orientation) {
  ...
  if (defined32(screen)) {
    if (defined32(screen.lockOrientation)) {
      locked = screen.lockOrientation(orientation);
    } else if (defined32(screen.mozLockOrientation)) {
      locked = screen.mozLockOrientation(orientation);
    } else if (defined32(screen.msLockOrientation)) {
      locked = screen.msLockOrientation(orientation);
    } else if (defined32(screen.orientation && screen.orientation.lock)) {
...
function unlockScreen() {
  const screen = window.screen;
  if (defined32(screen)) {
    if (defined32(screen.unlockOrientation)) {
      screen.unlockOrientation();
    } else if (defined32(screen.mozUnlockOrientation)) {
      screen.mozUnlockOrientation();
    } else if (defined32(screen.msUnlockOrientation)) {
      screen.msUnlockOrientation();
    } else if (defined32(screen.orientation && screen.orientation.unlock)) {
...
function VRButtonViewModel(scene, vrElement) {
  if (!defined32(scene)) {
    throw new DeveloperError26("scene is required.");
  }
  const that = this;
  ...
  this._vrElement = getElement16(vrElement) ?? document.body;
  this._callback = function() {
...
Object.defineProperties(VRButtonViewModel.prototype, {
  ...
  vrElement: {
    ...
    set: function(value) {
      if (!(value instanceof Element)) {
        throw new DeveloperError26("value must be a valid Element.");
      }
...
function VRButton(container, scene, vrElement) {
  if (!defined33(container)) {
    throw new DeveloperError27("container is required.");
  }
  if (!defined33(scene)) {
    throw new DeveloperError27("scene is required.");
  }
  container = getElement17(container);
...
import { Color as Color4, defined as defined34, JulianDate as JulianDate4 } from "@cesium/engine";
...
TimelineTrack.prototype.render = function(context, renderState) {
  ...
  if (JulianDate4.lessThan(startInterval, spanStart) && JulianDate4.greaterThan(stopInterval, spanStop)) {
    ...
    for (x = 0; x < renderState.timeBarWidth; ++x) {
      ...
      if (!defined34(start) && JulianDate4.greaterThanOrEquals(currentTime, startInterval)) {
        start = x;
      } else if (!defined34(stop2) && JulianDate4.greaterThanOrEquals(currentTime, stopInterval)) {
        stop2 = x;
      }
    }
...
function Timeline(container, clock) {
  ...
  if (!defined35(clock)) {
    throw new DeveloperError28("clock is required.");
  }
...
Timeline.prototype.zoomTo = function(startTime, stopTime) {
  if (!defined35(startTime)) {
    throw new DeveloperError28("startTime is required.");
  }
  if (!defined35(stopTime)) {
    throw new DeveloperError28("stopTime is required");
  }
  if (JulianDate5.lessThanOrEquals(stopTime, startTime)) {
    throw new DeveloperError28("Start time must come before end time.");
  }
...
Timeline.prototype.updateFromClock = function() {
  ...
  if (defined35(this._scrubElement)) {
    const seconds = JulianDate5.secondsDifference(
      this._scrubJulian,
      this._startJulian
    );
  ...
  if (defined35(this._timelineDragLocation)) {
...
globalThis.CESIUM_VERSION = "1.140.0";
export {
